...
Observe_matrix = np.zeros((n_outputs*iterations,n_states))

for i in range(0,iterations):
	Observe_matrix[i*n_outputs:(i+1)*n_outputs,:] = np.dot(C,np.linalg.matrix_power(Ad,i))

Observe_rank = matrix_rank(Observe_matrix)
print "\n(3) Observability Check:"
print "rank = " + str(Observe_rank)  # rank = 3 --> fully observable.

Observe_matrix_svd = np.linalg.svd(Observe_matrix)
OU = Observe_matrix_svd[0]
OV = np.transpose(Observe_matrix_svd[2])

zero_padder = np.zeros((n_states*iterations-n_states,n_states))
OS = np.concatenate(( np.diag(Observe_matrix_svd[1]), zero_padder),axis = 0)

# psuedo inverse
os = np.linalg.pinv(OS)

y_ = np.zeros((n_outputs*iterations,1))

storage = np.zeros((n_outputs,1))

for t in range(0,iterations):
	for tau in range(0, t):
		storage = storage+np.dot(np.dot(C,np.linalg.matrix_power(Ad,t-tau-1)),np.dot(Bd,u[n_inputs*tau:(tau+1)*n_inputs,:]))
	Cx = np.expand_dims(np.dot(C,x_[:,t]),axis =1)
	y_[n_outputs*t:(t+1)*n_outputs,:] = Cx - storage
	storage = np.zeros((n_outputs,1))

Observe_matrix_inv = np.dot(np.dot(OV,os),np.transpose(OU))
Predicted_Initial_State = np.dot(Observe_matrix_inv,y_)
print "Predicted initial condition: " + str(np.transpose(Predicted_Initial_State)) 
print "Actual initial condition:    " + str(Xo) 
# --------------------------------------------------------------
